...
Each person edits only their own files. One shared file, frozen after Phase 0.
...
2. Write `src/lib/contract.ts` together (draft below).
3. Write `src/lib/fixtures.ts` — four hardcoded results: healthy cat, painful cat,
   rejected image, partial-scorable (whiskers not visible).
4. Stub `src/lib/assess/index.ts` to return a fixture after a 1.5s delay.
5. Confirm `npm run dev` runs on both machines.
6. **Commit and push to `main` before anyone branches.**
...
export const ANALGESIA_THRESHOLD = 0.39; // Evangelista et al. 2019
export const MIN_SCORABLE_AUS = 3;       // below this, refuse to report a score
